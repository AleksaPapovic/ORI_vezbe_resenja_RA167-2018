...
class State(object):
    ...
    @abstractmethod
    def __init__(self, board, parent=None, position=None, goal_position=None,food=None,old_goal_position=None):
        """
        ...
        if self.parent is None:  # ako nema roditeljsko stanje, onda je ovo inicijalno stanje
            ...
            self.food = board.find_position(self.get_pfood_code())
    ...
    def get_next_states(self):
        ...
        for new_position in new_positions:
            next_state = self.__class__(self.board, self, new_position, self.goal_position ,self.food,self.old_goal_position)
...
class RobotState(State):
    def __init__(self, board, parent=None, position=None, goal_position=None,food =None,old_goal_position=None):
        super(self.__class__, self).__init__(board, parent, position, goal_position,food,old_goal_position)
    ...
    def is_yellow(self):
        x, y = self.position
        if self.board.data[x][y] == 'y':
            return True

    def set_start(self):
        x, y = self.position
        niz = self.board.find_yfood_positions(self.get_yfood__code())
        it = 0
        zute = deque([])
        if niz is not None and self.board.data[x][y] == 'y':
            for item in niz:
                zute.append(item)
                it = it + 1
            print("it")
            ...
            print("redni x")
            print(r)
            self.position = zute.__getitem__(r-2) ,zute.__getitem__(r-1)
            return

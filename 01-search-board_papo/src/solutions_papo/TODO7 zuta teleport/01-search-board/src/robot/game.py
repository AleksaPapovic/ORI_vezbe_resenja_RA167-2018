...
def do_search():
    ...
    #search = IterativeDepthFirstSearch(board,100,1)
    search = Teleportuj(board)
...
def debug():
    ...
    for idx, p in enumerate(processed):
        # rp ,cp = p.position  linija za lepsi prikaz idfs-a
        # board.data[rp][cp] = 'b' linija za lepsi prikaz idfs-a
